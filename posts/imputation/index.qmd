...
# Single imputation {#sec-single-imputation}
...
Somewhat related to @sec-multiple-imputation in that you impute multiple times, bootstrap imputation is typically simpler for the analyst to setup.  The basic idea is:

1. Choose one of the "single imputation methods" from @sec-single-imputation
2. Do simple nonparametric bootstrap resampling with replacement to create multiple "bootstrap versions" of your dataset
3. Impute each bootstrap version of the dataset using the method chosen in step 1
4. Analyze each imputed bootstrap dataset separately then pool the results for statistical inference.  Standard bootstrap inference methods apply here (e.g. percentile confidence intervals, [bootstrap-pvalues](https://cran.r-project.org/web/packages/boot.pval/index.html), etc.)
